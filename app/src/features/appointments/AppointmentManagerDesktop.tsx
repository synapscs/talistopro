...
import { Calendar as CalendarIcon, Clock, Plus, Loader2, List, Grid, X, MessageSquare, ChevronRight, User, Car, Bell, Edit, Trash2, ArrowRight } from 'lucide-react';
...
import { format, isSameDay, startOfDay, endOfDay, startOfWeek, endOfWeek, addDays, isToday } from 'date-fns';
...
export const AppointmentManagerDesktop = () => {
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" onClick={() => setShowDetail(false)}>
                    <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-6 border-t border-slate-200 dark:border-slate-800 space-y-3">
                            {!selectedAppointment.serviceOrderId && selectedAppointment.status !== 'COMPLETED' && selectedAppointment.status !== 'CANCELLED' && (
                                <button
                                    onClick={() => {
                                        setShowDetail(false);
                                        setShowConvertModal(true);
                                    }}
                                    className="w-full py-3 bg-primary-600 hover:bg-primary-500 text-white rounded-xl font-bold text-sm transition-all flex items-center justify-center space-x-2"
                                >
                                    <ArrowRight size={16} />
                                    <span>Convertir a {terminology.orderLabel}</span>
                                </button>
                            )}

                            {selectedAppointment.serviceOrderId && (
                                <button
                                    onClick={() => navigate(`../orders/${selectedAppointment.serviceOrderId}`)}
                                    className="w-full py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold text-sm transition-all flex items-center justify-center space-x-2"
                                >
                                    <span>Ver {terminology.orderLabel} #{selectedAppointment.serviceOrder?.orderNumber || 'vinculada'}</span>
