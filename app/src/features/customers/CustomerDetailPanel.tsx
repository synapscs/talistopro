...
import { X, Phone, Mail, MapPin, MessageSquare, Plus, Package, ChevronRight, User, ExternalLink, ShieldCheck, Edit } from 'lucide-react';
...
export const CustomerDetailPanel = ({ customer, onClose, onAddAsset, onEditAsset, onCreateOrder }: CustomerDetailPanelProps) => {
    ...
    const { data: assets, isLoading } = useAssets(customer.id);
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <a
                        ...
                        className="flex flex-col items-center justify-center p-4 bg-white dark:bg-slate-900 border border-slate-300/40 dark:border-slate-800 rounded-2xl hover:border-primary-500/30 hover:shadow-lg transition-all group"
                    >
                        <Phone size={20} className="text-primary-600 mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Llamar</span>
                        <span className="text-xs font-black text-slate-900 dark:text-white mt-1">{customer.phone}</span>
                    </a>
                    <a
                        href={`https://wa.me/${customer.phone.replace(/[^0-9]/g, '')}`}
                        target="_blank"
                        rel="noreferrer"
                        className="flex flex-col items-center justify-center p-4 bg-white dark:bg-slate-900 border border-slate-300/40 dark:border-slate-800 rounded-2xl hover:border-emerald-500/30 hover:shadow-lg transition-all group"
                    >
                        <MessageSquare size={20} className="text-emerald-600 mb-2 group-hover:scale-110 transition-transform" />
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">WhatsApp</span>
                        <span className="text-xs font-black text-slate-900 dark:text-white mt-1 text-center truncate w-full">Chat Directo</span>
                    </a>
                </div>
